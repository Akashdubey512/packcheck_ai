...
/**
 * Escapes special HTML characters to prevent XSS and HTML injection in PDF renderer.
 */
...
/**
 * Construct HTML report supporting canonical Inspection documents and legacy Scans with strict HTML sanitization.
 */
export function buildReportHtml(scan) {
  const rawId = scan.inspectionId || scan.id || scan._id || "INSP-RECORD";
  const inspectionId = escapeHtml(rawId);
  const rawStatus = scan.status || scan.overallStatus || scan.compliance?.overallStatus || "REVIEW_REQUIRED";
  const statusUpper = escapeHtml(String(rawStatus).toUpperCase());
  const timestamp = escapeHtml(
    scan.createdAt ? new Date(scan.createdAt).toLocaleString() : new Date().toLocaleString()
  );
  ...
  if (scan.fields && typeof scan.fields === "object") {
    Object.entries(scan.fields).forEach(([key, f]) => {
      const label = escapeHtml(DECLARATION_LABELS[key] || f.label || key);
      ...
      const val = escapeHtml(f.normalizedValue || f.rawValue || "-");
      const statusText = isPresent ? "✅ Compliant" : "❌ Missing";
      const conf = escapeHtml(f.confidence ? `${Math.round(f.confidence * 100)}%` : "-");
      rows.push(`<tr>
    ...
    Object.entries(scan.declarations).forEach(([key, d]) => {
      const label = escapeHtml(DECLARATION_LABELS[key] || key);
      const isPresent = Boolean(d.found);
      const val = escapeHtml(d.value || "-");
      const statusText = isPresent ? "✅ Present" : "❌ Missing";
      const conf = escapeHtml(d.confidence ? `${Math.round(d.confidence * 100)}%` : "-");
      rows.push(`<tr>
        <td><strong>${label}</strong></td>
        <td><span class="badge ${isPresent ? "badge-pass" : "badge-fail"}">${statusText}</span></td>
        <td>${val}</td>
  ...
  const violations = rawViolations.map((v) =>
    escapeHtml(typeof v === "string" ? v : v.message || v.rule_name || JSON.stringify(v))
  );
  ...
  const sha256 = escapeHtml(rawSha);
...
<body>
  ...
  <div class="footer">
    ...
    <p>This document is an evidence-backed statutory evaluation report generated under the Legal Metrology (Packaged Commodities) Rules, 2011. It reflects automated OCR extraction and rule evaluations, subject to regulatory officer review.</p>
  </div>
</body>
</html>`;
}
...
/**
 * Generate PDF buffer using Puppeteer headless Chrome with SSRF and XSS defenses.
...
export async function generateReportPdf(scan) {
  ...
  try {
    ...
    // Disable JavaScript execution in headless renderer for SSRF / RCE defense
    await page.setJavaScriptEnabled(false);

    // Strict request interception blocking external outbound HTTP calls (SSRF defense)
